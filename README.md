...
FIFA World Cup, with a live search. Built as a **system under test** for the
QAcart stubbing & mocking lessons.
...
The only endpoint. No auth, read-only.

**`GET /api/v1/teams`** → `200 { "teams": Team[] }` — all 48 teams, A→Z.

**`GET /api/v1/teams?search=<term>`** → the matching subset.
